...
// database *writes* are opt-in, defaulting to `false`
// (`packages/frontend/core/src/modules/feature-flag/constant.ts`), which is
...
// This is a pre-existing, deliberate product decision this phase must not
// touch -- but a spec that wants to exercise the *writable* mobile path
// (to prove the handle's CSS-visibility fix, not its JS gate) must opt in
// the same way a real device with the setting enabled would.

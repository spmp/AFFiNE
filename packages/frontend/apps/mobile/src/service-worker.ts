...
//
// Identical logic to packages/frontend/apps/web/src/service-worker.ts,
// duplicated rather than shared — both apps needed this, and there wasn't
// an existing shared location between just these two targets (not
// @affine/core, which is also used by electron/desktop/ios/android, none
// of which should get a service worker). Worth factoring out into a real
// shared module as a follow-up; kept as a duplicate for now to get mobile
// working on the same footing as web without a bigger restructuring.
...
// to cache-first indefinitely. Matches by extension alone, not by path
// prefix (e.g. /js/) — lazy CSS chunks (npm-async-@vanilla-extract...) are
// emitted at the root, not under /js/, and a path-scoped pattern misses
// them entirely.
//
...
async function cacheFirst(
  ...
  // no-store for the same reason as the install handler below: this is the
...
self.addEventListener('fetch', event => {
  ...
  // runtime pattern is broad enough (any /js/*.js) to also match the
  ...
  if (request.mode === 'navigate') {
    ...
    // as a desktop UA hitting this same URL) can satisfy this fetch without
      ...
      fetch(request.url, { cache: 'no-store' })
        .then(response => {
          ...
          if (response.ok) {
            ...
            // once any consumption has started.
